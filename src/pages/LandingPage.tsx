...
export default function LandingPage({ vehicles, onBook, onViewFleet, onViewAircraft, onContact, onInquire, onNav }: LandingPageProps) {
    ...
    <div className="bg-black-rich min-h-screen">
      ...
      <section className="py-32 px-6 md:px-12 border-b border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-20 space-y-8 md:space-y-0 text-center md:text-left">
            ...
            <button onClick={onViewFleet} className="px-10 py-4 border border-white/10 rounded-sm text-[11px] tracking-[0.4em] font-medium hover:bg-white hover:text-black transition-all duration-500 uppercase">
              Browse Full Fleet
            </button>
          </div>
          <div className="grid md:grid-cols-3 gap-12">
            {featuredVehicles.map((vehicle, idx) => (
              <motion.div key={vehicle.id} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: idx * 0.1 }}>
                <VehicleCard vehicle={vehicle} onSelect={(v) => console.log('Select', v)} onBook={(v, color) => onBook(v, color)} onInquire={onInquire} />
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 gap-20 items-center">
            ...
            <div className="order-1 md:order-2 space-y-10">
              ...
              <p className="text-silver text-[18px] font-light leading-relaxed tracking-wide">Access a curated fleet of Standard, Mid-tier, and Presidential class jets. Our aviation partners provide global reach with absolute discretion.</p>
              ...
              <button onClick={onViewAircraft} className="bg-white text-black px-12 py-4 rounded-sm text-[11px] tracking-[0.3em] font-bold hover:bg-gold hover:text-white transition-all shadow-xl uppercase">View Aircraft</button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-40 px-6 text-center border-b border-white/5 bg-gradient-to-b from-black-pure to-black-rich">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto">
          <p className="font-serif text-3xl md:text-5xl italic text-white leading-snug mb-12">"APPROVED was built for those who understand that true luxury is not status displayed to the public — it is access granted to a select few."</p>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 gap-20 items-center">
            <div className="space-y-10">
              ...
              <p className="text-silver text-[18px] font-light leading-relaxed tracking-wide">From armed close protection to presidential escort convoys. Your safety is managed with tactical precision and absolute confidentiality.</p>
              <button onClick={onContact} className="border border-white/20 text-white px-12 py-4 rounded-sm text-[11px] tracking-[0.4em] font-bold hover:bg-white hover:text-black transition-all uppercase">Security Enquiries</button>
            </div>
            <div>
      ...
      <section className="py-40 px-6 text-center bg-black-pure relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="max-w-4xl mx-auto relative z-10">
          <h2 className="font-serif text-5xl md:text-7xl mb-12 italic tracking-tight text-white">Entry into Exclusivity</h2>
          <p className="text-silver text-[16px] md:text-[18px] mb-16 font-light leading-relaxed tracking-widest max-w-2xl mx-auto uppercase">NOT ADVERTISED. NOT ACCESSIBLE. <br /> NOT FOR EVERYONE.</p>
          <div className="flex flex-col md:flex-row items-center justify-center space-y-6 md:space-y-0 md:space-x-10">
            <button onClick={onViewFleet} className="bg-white text-black px-16 py-5 rounded-sm text-[12px] tracking-[0.4em] font-bold hover:bg-gold hover:text-white transition-all duration-500 w-full md:w-auto shadow-2xl">BROWSE FLEET</button>
            <button onClick={onContact} className="bg-transparent text-white px-16 py-5 rounded-sm text-[12px] tracking-[0.4em] font-medium hover:text-gold hover:bg-white/10 transition-all duration-500 w-full md:w-auto uppercase flex items-center justify-center gap-3">
      ...
      <footer className="py-32 px-6 md:px-12 border-t border-white/5 bg-black-pure relative z-20">
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-20">
          <div className="md:col-span-1">
            ...
            <p className="text-silver text-[14px] font-light leading-relaxed tracking-wide italic">Exclusivity by design. Discretion by demand. We represent individuals who operate at the highest level.</p>
          </div>
          <div>
            <h4 className="text-[11px] tracking-[0.4em] font-bold mb-10 uppercase text-white">Directory</h4>
            <ul className="space-y-5">
              {[
                { label: 'Aviation', page: 'aviation' },
                { label: 'Fleet', page: 'fleet' },
                { label: 'Security', page: 'security' },
              ].map(item => (
                <li key={item.label}>
                  <button onClick={() => onNav(item.page)} className="text-silver text-[12px] tracking-[0.2em] hover:text-gold transition-colors uppercase font-medium">{item.label}</button>
                </li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-5">
              {[
                ...
                { label: 'Confidentiality', page: 'confidentiality' },
              ].map(item => (
                <li key={item.label}>
                  <button onClick={() => onNav(item.page)} className="text-silver text-[12px] tracking-[0.2em] hover:text-gold transition-colors uppercase font-medium">{item.label}</button>
                </li>
          ...
          <div>
            ...
            <div className="flex bg-[#0f0f0f] border border-white/5 p-1 rounded-full group focus-within:border-gold/30 transition-all">
              <input type="email" placeholder="EMAIL@CLIENT.COM" className="bg-transparent border-none text-[10px] px-8 focus:ring-0 w-full tracking-[0.2em]" />
              <button className="bg-white text-black px-8 py-3 rounded-full text-[10px] font-bold tracking-[0.2em] hover:bg-gold hover:text-white transition-all uppercase">Send</button>
            </div>
          </div>
        </div>
        <div className="max-w-7xl mx-auto pt-32 text-center border-t border-white/5 mt-32 flex flex-col md:flex-row justify-between items-center space-y-6 md:space-y-0">
          ...
          <div className="flex space-x-12">
            {[
              ...
              { label: 'CONDUCT', page: 'conduct' },
            ].map(l => (
              <button key={l.label} onClick={() => onNav(l.page)} className="text-[9px] tracking-[0.3em] text-dim hover:text-gold transition-colors uppercase font-bold">{l.label}</button>
            ))}
          </div>
        </div>
      </footer>
